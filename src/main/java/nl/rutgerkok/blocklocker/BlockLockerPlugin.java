...
public interface BlockLockerPlugin {
  ...
  /**
   * Gets the {@link ChestSettings} object, containing all customizable aspects of the plugin.
   *
  ...
  /**
   * Gets the combined group system of the plugin, which can be used to add other group systems.
   *
  ...
  /**
   * Gets the location checkers, which are used to prevent players from placing chests in the
   * wilderness.
   *
  ...
  /**
   * Gets the protection updater, used to mark protections as needing an update, for example for
   * fixing missing UUIDs.
   *
   ...
   *
   * @param block Used to run the task in the region of the block (when using the Folia server).
   ...
   * @param block Used to run the task in the region of the block (when using the Folia server).
   * @param runnable The task.
   * @param ticks In how many ticks the method needs to run.
   */
  void runLater(Block block, Runnable runnable, int ticks);

  /**
   * Runs a task on the server thread. On Folia, this thread only controls things that don't belong
   * to a specific region, like weather and time.
   *
   * @param runnable The task.
   * @param ticks In how many ticks the method needs to run.
   */
