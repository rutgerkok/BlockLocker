...
public interface ProtectionFinder {
    ...
    /**
     * Gets whether this block can be protected by a sign. This can either be
     * because it is itself a block that can be locked (like a chest) or because it
     * supports a block that is protectable (like the block below a door). Note that
     * this method returns false for signs, since you cannot protect a sign with
     * another sign. See {@link #isSignNearbyProtectable(Block)} for that case.
     * 
     * @param block
     *            The block to check.
