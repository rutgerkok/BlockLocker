 ...
 * Finds a protection.
 *
 * <p>See {@link SearchMode} for a description of protection blocks, supporting blocks and sign
 * blocks.
 */
class ProtectionFinderImpl implements ProtectionFinder {
   ...
   *
   * @param sign The sign, whether properly formatted or not.
   * @return A block that can be protected, and may or may not be protected currently.
   */
   ...
   *
   * @param supportingBlock The block that is supporting a protection block.
   ...
   *
   * @param block The protection/supporting block.
   * @param searchMode How the search is carried out.
  ...
  private Optional<Protection> findProtectionForProtectionBlock(Block protectionBlock) {
    ...
    switch (protectionType.get()) {
      case CONTAINER:
        ...
        return Optional.of(ContainerProtectionImpl.fromBlocksWithSigns(signs, blocks, blockFinder));
      case DOOR:
        CompleteDoor door = new CompleteDoor(protectionBlock);
        Collection<ProtectionSign> doorSigns =
            blockFinder.findAttachedSigns(door.getBlocksForSigns());
        if (doorSigns.isEmpty()) {
          return Optional.empty();
        }
        return Optional.of(DoorProtectionImpl.fromDoorWithSigns(doorSigns, blockFinder, door));
      case ATTACHABLE:
        Collection<ProtectionSign> trapDoorSigns =
            blockFinder.findAttachedSigns(
                Arrays.asList(protectionBlock, blockFinder.findSupportingBlock(protectionBlock)));
        if (trapDoorSigns.isEmpty()) {
          return Optional.empty();
        }
        return Optional.of(
            AttachedProtectionImpl.fromBlockWithSigns(trapDoorSigns, blockFinder, protectionBlock));
      default:
        throw new UnsupportedOperationException(
            "Don't know how to handle protection type " + protectionType.get());
    }
  }

  /**
   * Gets the {@link Protection} for the given block, which is already part of a protection; it must
   * be protection block, not a supporting block or a sign. A sign is given as a hint to this
   * method, so that the {@link Protection#getOwner()} method executes a little faster.
   *
   * @param protectionBlock The block that represents the protection (is a door or container block).
   * @param sign The sign used for finding the block.
   * @return The created protection.
   */
  private Optional<Protection> findProtectionForProtectionBlock(
      Block protectionBlock, ProtectionSign sign) {
    Optional<ProtectionType> protectionType = settings.getProtectionType(protectionBlock);
    if (!protectionType.isPresent()) {
      return Optional.empty();
    }
    ...
    switch (protectionType.get()) {
      case CONTAINER:
        ...
        return Optional.of(ContainerProtectionImpl.fromBlocksWithSign(sign, blocks, blockFinder));
      case DOOR:
        CompleteDoor door = new CompleteDoor(protectionBlock);
        return Optional.of(DoorProtectionImpl.fromDoorWithSign(sign, blockFinder, door));
      case ATTACHABLE:
        return Optional.of(
            AttachedProtectionImpl.fromBlockWithSign(sign, blockFinder, protectionBlock));
      default:
        throw new UnsupportedOperationException(
            "Don't know how to handle protection type " + protectionType.get());
    }
  ...
  public ProtectionSign newProtectionSign(Sign sign, SignType signType, Profile onFirstLine) {
    return new ProtectionSignImpl(
        sign.getLocation(), signType, Collections.singletonList(onFirstLine), false);
  }
}
