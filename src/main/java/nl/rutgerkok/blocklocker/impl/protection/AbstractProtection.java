...
abstract class AbstractProtection implements Protection {
  ...
  /**
   * Constructor for creating the protection with all signs already looked up. Collection may not be
   * empty.
   *
  ...
  /**
   * Constructor for creating the protection with just a single sign looked up. Not all signs are
   * found yet. If it turns out that all signs are needed, {@link #fetchSigns()} will be called.
   *
  ...
  /**
   * Checks if the instance is of {@link Openable} that will open correctly when you call {@link
   * Openable#setOpen(boolean)}. For barrels, opening them using {@link Openable#setOpen(boolean)}
   * won't show the inventory, so those shouldn't be considered openable.
   *
  ...
  protected static boolean isFunctionalOpenable(BlockData blockData) {
    ...
    if (blockData.getMaterial() == Material.BARREL) {
      return false; // Barrels are openable in Bukkit since 1.19, but then the inventory won't show
      // up
    }
    return true;
  }
  ...
  /**
   * Finds all signs attached to this protection. This method must always do a lookup, results may
   * not be cached: {@link AbstractProtection} already does that.
   *
   * @return All signs.
   */
