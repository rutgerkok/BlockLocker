...
/**
 * Reads a single sign for profiles. Doesn't verify the sign header, the first line is simply
 * skipped for reading.
 */
class SignParserImpl implements SignParser {

  private static final NamespacedKey HEADER_KEY = NbtSecretSignEntry.key("header");
  private static final NamespacedKey[] PROFILE_KEYS = {
    NbtSecretSignEntry.key("profile_1"),
    NbtSecretSignEntry.key("profile_2"),
    NbtSecretSignEntry.key("profile_3"),
    NbtSecretSignEntry.key("profile_4"),
    NbtSecretSignEntry.key("profile_5"),
  ...
  private SignType getSignTypeOrNull(String header) {
    ...
    for (SignType type : SignType.values()) {
      if (chestSettings.getSimpleLocalizedHeaders(type).stream()
          .anyMatch(localizedHeader -> trimmed.equalsIgnoreCase(localizedHeader))) {
        return type;
      }
    }
    return null;
  ...
  private Optional<ProtectionSign> parseAdvancedSign(Sign sign) {
    ...
    int i = 0;
    boolean signHadDataMismatch =
        false; // Set to true when the display text doesn't match the stored data
    ...
    return Optional.of(
        new ProtectionSignImpl(
            sign.getLocation(), type, profiles, signHadDataMismatch || headerMismatch));
  }
   ...
   *
   * @param location The location on the sign.
   * @param frontLines The lines on the front of the sign.
   * @param backLines The lines on the back of the sign.
   * @return The protection sign, if the header format is correct.
   */
  private Optional<ProtectionSign> parseSimpleSign(
      Location location, String[] frontLines, String[] backLines) {
    ...
    ProtectionSignImpl protectionSignImpl =
        new ProtectionSignImpl(location, signType, profiles, true);
  ...
  public void saveSign(ProtectionSign sign) {
    ...
    frontSide.setLine(
        0, chestSettings.getFancyLocalizedHeader(sign.getType(), frontSide.getLine(0)));
    if (profiles.size() > 3) {
      backSide.setLine(
          0, chestSettings.getFancyLocalizedHeader(SignType.MORE_USERS, backSide.getLine(0)));
    }
    ...
    for (Profile profile : profiles) {
      ...
      NbtSecretSignEntry signEntry =
          new NbtSecretSignEntry(data.getAdapterContext().newPersistentDataContainer());
