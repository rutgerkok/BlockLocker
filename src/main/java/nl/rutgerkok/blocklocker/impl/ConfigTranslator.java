...
class ConfigTranslator extends Translator {
  ...
  /**
   * Little class that contains multiple translated values. This class extends a translationValue
   * which is the primary value, it contains a list with all the other possible values.
   */
  ...
  /**
   * Little class to hold the different representations of the translated values. The original
   * ({@code &1foo}), the colored ({@code §1foo}) and uncolored ({@code foo}).
   */
  ...
  /**
   * Checks if all values in the list are strings. Returns false if not a list or if the list
    ...
    for (Entry<Translation, TranslationValue> translationEntry : translations.entrySet()) {
      ...
      if (values.size() == 1) {
        ...
        List<String> stringValues =
            values.stream().map(value -> value.original).collect(Collectors.toList());
