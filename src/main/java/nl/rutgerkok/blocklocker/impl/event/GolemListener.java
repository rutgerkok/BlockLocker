...
/**
 * Listener for just golems. Separated to allow the plugin to run on older Minecraft versions. In
 * the future, when backwards compatibility is no longer necessary, the hopper event could be
 * handled here too, then it would be an ExtractListener or something.
 */
public final class GolemListener extends EventListener {
  ...
  public void onEntityContainerTarget(ItemTransportingEntityValidateTargetEvent event) {
    ...
    Optional<Protection> protection =
        plugin.getProtectionFinder().findProtection(block, SearchMode.MAIN_BLOCKS_ONLY);
