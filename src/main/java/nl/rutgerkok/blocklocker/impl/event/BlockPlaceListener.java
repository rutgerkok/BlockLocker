...
public final class BlockPlaceListener extends EventListener {
   ...
   * @param block A block that's part of the protection.
   * @return True if there is a protection, but the player is not the owner. False otherwise. Still
   *     returns false if the player is allowed, but not the owner.
   */
    ...
    if (interferingProtection.isPresent()) {
      ...
      if (!event.getPlayer().hasPermission(Permissions.CAN_ADMIN)) {
        plugin
            ...
            .sendMessage(
                player,
                Translation.PROTECTION_NO_ACCESS,
                interferingProtection.get().getOwnerDisplayName());
        ...
        plugin
            ...
            .sendMessage(
                player,
                Translation.PROTECTION_BYPASSED,
                interferingProtection.get().getOwnerDisplayName());
        return;
      }
    }
    ...
    if (block.getType().equals(Material.CHEST) || block.getType().equals(Material.TRAPPED_CHEST)) {
      ...
      for (BlockFace searchFace : BlockFinder.NORTH_EAST_SOUTH_WEST_UP_DOWN) {
        ...
        Optional<Protection> protectionBySomeoneElse =
            getProtectionBySomeoneElse(player, nearBlock);
  ...
  private Optional<Protection> willInterfereWithStandard(Player player, Block block) {
    ...
    if (block.getType().equals(Material.CHEST) || block.getType().equals(Material.TRAPPED_CHEST)) {
      ...
      if (player.isSneaking()) {
        return Optional
            .empty(); // If a player is sneaking, the chest will not connect, so it's safe
      }
      for (BlockFace searchFace : BlockFinder.CARDINAL_FACES) {
        Block nearBlock = block.getRelative(searchFace);
        ...
        boolean alreadyADoubleChest =
            (((InventoryHolder) nearBlock.getState()).getInventory().getHolder()
        ...
        boolean areSiblings =
            (faceOfNearBlock == faceOfBlock)
                && faceOfBlock != searchFace
