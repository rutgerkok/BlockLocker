...
final class Config {
  ...
  Config(Plugin plugin) {
    ...
    protectableMaterialsMap.put(
        ProtectionType.DOOR, readMaterialSet(config.getStringList(Key.PROTECTABLE_DOORS)));
  ...
  private void writeToConfig(FileConfiguration config) {
    ...
    config.set(
        Key.ALLOW_DESTROY_BY, this.allowDestroyBy.stream().map(AttackType::toString).toList());
    config.set(
        Key.PROTECTABLE_CONTAINERS,
        writeMaterialSet(protectableMaterialsMap.get(ProtectionType.CONTAINER)));
    config.set(
        Key.PROTECTABLE_DOORS, writeMaterialSet(protectableMaterialsMap.get(ProtectionType.DOOR)));
    config.set(
        Key.PROTECTABLE_ATTACHABLES,
        writeMaterialSet(protectableMaterialsMap.get(ProtectionType.ATTACHABLE)));
  }
   ...
   * @return True if the material can be protected by the given type, false otherwise.
   */
   ...
   *
   * @param type Protection type that must be checked for being able to protect this type.
   * @param block Block to check.
   * @return True if the material can be protected by the given type, false otherwise.
   */
  ...
  /**
   * Gets the amount of days that a chest owner has to be offline for a chest to expire. 0 or
   * negative means that chests never expire.
   *
  ...
  /**
   * Gets the default amount of ticks a door stays open before automatically closing. 0 or negative
   * values will make the door never close automatically.
   *
  ...
  /**
   * Transforms the string collection into a material set, by parsing each string using {@link
   * Material#matchMaterial(String)}. The resulting set will be mutable. All strings that cannot be
   * parsed are logged and then ignored.
   *
