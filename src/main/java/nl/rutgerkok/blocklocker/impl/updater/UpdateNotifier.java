 ...
 *
 * <p>When registered as a listener, the notification will be send to all admins that are logging
 * in.
 */
final class UpdateNotifier implements Listener {
   ...
   *
   * @param sender The person to send to.
   */
  void sendNotification(CommandSender sender) {
    ...
    switch (result.getStatus()) {
      ...
      case UNSUPPORTED_SERVER:
        ...
        translator.sendMessage(
            sender, Translation.UPDATER_UNSUPPORTED_SERVER, newVersion, mcVersionsString);
        break;
      default:
    ...
    if (infoUrl.isPresent()) {
      translator.sendMessage(
          sender, Translation.UPDATER_MORE_INFORMATION, infoUrl.get().toString());
    }
  }
}
