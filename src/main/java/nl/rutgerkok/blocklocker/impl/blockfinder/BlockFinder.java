...
/**
 * Finds blocks that logically belong together, like the other half of a chest, the attached signs,
 * etc.
 */
public abstract class BlockFinder {

  public static final BlockFace[] CARDINAL_FACES = {
  ...
  public static final BlockFace[] NORTH_EAST_SOUTH_WEST_UP_DOWN = {
  ...
  /**
   * Gets the block that supports the given block. If the returned block is destroyed, the given
   * block is destroyed too.
   *
   * <p>For blocks that are self-supporting (most blocks in Minecraft), the method returns the block
   * itself.
   *
  ...
  /**
   * Checks if the sign at the given position is attached to the container. Doens't check the text
   * on the sign.
   *
   * @param signBlock The block that is a sign.
   * @param attachedTo The block the sign must be attached to. If this is not the case, the method
   *     returns false.
   * @return True if the direction and header of the sign are valid, false otherwise.
   */
