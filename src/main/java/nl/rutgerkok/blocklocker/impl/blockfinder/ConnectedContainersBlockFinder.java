...
final class ConnectedContainersBlockFinder extends BlockFinder {
  ...
  public List<Block> findContainerNeighbors(Block block) {
    ...
    if (chestNeighborFace != null) {
      ...
      BlockFace[] searchDirections = {
        this.turn90Degrees(chestNeighborFace),
   ...
   *
   * @param containerMaterial The material {@link Block#getType() startingBlock.getType()} returns.
   * @param startingBlock The starting block.
   * @param blocks All connected blocks above and below (so not the starting block itself) of the
   *     same type will be added to this list.
   */
