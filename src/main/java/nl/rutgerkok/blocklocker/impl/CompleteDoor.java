...
/**
 * Represents a single or double door. Used to find all blocks of a door, as well to open or close a
 * door.
 */
public final class CompleteDoor {
   ...
   *
   * @param doorBlock A block that is part of the door.
   */
  public CompleteDoor(Block doorBlock) {
    ...
    if (hinge == Hinge.LEFT) {
      ...
      topRightBlock =
          asDoorBlockOrNull(getRelativeOrNull(topBlock, faceToRightDoor), Hinge.RIGHT, doorFace);
      bottomRightBlock =
          asDoorBlockOrNull(getRelativeOrNull(bottomBlock, faceToRightDoor), Hinge.RIGHT, doorFace);
      ...
      topLeftBlock =
          asDoorBlockOrNull(getRelativeOrNull(topBlock, faceToLeftDoor), Hinge.LEFT, doorFace);
      bottomLeftBlock =
          asDoorBlockOrNull(getRelativeOrNull(bottomBlock, faceToLeftDoor), Hinge.LEFT, doorFace);
    } else {
      topLeftBlock = null;
      topRightBlock = null;
  ...
  private @Nullable Block asDoorBlockOrNull(
      @Nullable Block nullableBlock,
      @Nullable Hinge expectedHinge,
  ...
  /**
   * Gets a collection of all blocks where attached protection signs are used for this door. This
   * includes the blocks above and below the door, and the blocks on the left and right of the door.
   *
  ...
  /**
   * Gets whether the door is currently open. The result is undefined if the door is half-open,
   * half-closed.
   *
   ...
   *
   * @param open Whether the door must be opened (true) or closed (false).
   * @param soundAction Whether a sound must be played.
   */
